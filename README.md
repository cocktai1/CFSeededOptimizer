...
**Q: 为什么 Mac 上测得 300ms，而 iPhone 上接近 1000ms？**

A: 常见原因有三个：
1. iPhone 的网络栈和电源策略更激进，TLS/HTTP 握手开销通常更明显。
2. iPhone 上的 DNS/IPv6 路由可能和 Mac 不完全一致，Anycast 命中点不同。
3. 如果你在 iPhone 上开了省电模式、iCloud Private Relay、VPN 或 Wi-Fi 私有地址，路径会进一步变化。

这也是为什么脚本现在改成了更接近 cf_speedup 的 HTTP 直测，并允许你在有业务探针时再做第二层筛选。
